 ...
 */
public class LCBVideoPlayer extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener, SurfaceHolder.Callback, View.OnTouchListener {
    ...
    public LCBVideoPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//        surfaceView.setBackgroundColor(R.color.black_a10_color);
    ...
    public void setUp(String url, String thumb, String title, boolean ifShowTitle) {
        ...
        Utils.loadImage(context,thumb,ivThumb);
//        ImageLoader.getInstance().displayImage(thumb, ivThumb, Utils.getDefaultDisplayImageOption());
        CURRENT_STATE = CURRENT_STATE_NORMAL;
        setTitleVisibility(View.VISIBLE);
        if (uuid.equals(LCBMediaManager.intance().uuid)) {
            LCBMediaManager.intance().mediaPlayer.stop();
        }
...
//            surfaceView.setBackgroundColor(R.color.black_a10_color);
        ...
        if (!LCBMediaManager.intance().uuid.equals(uuid)) {
        ...
        if (videoEvents.type == VideoEvents.VE_PREPARED) {
            ...
            LCBMediaManager.intance().mediaPlayer.setDisplay(surfaceHolder);
            LCBMediaManager.intance().mediaPlayer.start();
            pbLoading.setVisibility(View.INVISIBLE);
            if (!ifMp3) {
                ivCover.setVisibility(View.INVISIBLE);
            }
            ...
            int mVideoWidth = LCBMediaManager.intance().currentVideoWidth;
            int mVideoHeight = LCBMediaManager.intance().currentVideoHeight;
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start || i == R.id.thumb) {
            ...
            if (CURRENT_STATE == CURRENT_STATE_NORMAL) {
                LCBMediaManager.intance().clearWidthAndHeight();
                ...
                setProgressAndTime(0, 0, 0);
                setProgressBuffered(0);
                LCBMediaManager.intance().prepareToPlay(getContext(), url);
                LCBMediaManager.intance().setUuid(uuid);
                ...
                LCBMediaManager.intance().mediaPlayer.pause();
            ...
            if (ifFullScreen) {
                ...
                LCBMediaManager.intance().mediaPlayer.pause();
                LCBMediaManager.intance().mediaPlayer.setDisplay(null);
                LCBMediaManager.intance().backUpUuid();
    ...
    private void setProgressAndTimeFromTimer() {
        int position = LCBMediaManager.intance().mediaPlayer.getCurrentPosition();
        int duration = LCBMediaManager.intance().mediaPlayer.getDuration();
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        if (fromUser) {
            int time = progress * LCBMediaManager.intance().mediaPlayer.getDuration() / 100;
            LCBMediaManager.intance().mediaPlayer.seekTo(time);
            pbLoading.setVisibility(View.VISIBLE);
            ivStart.setVisibility(View.INVISIBLE);
        }
    }
...
//        cancelDismissControlViewTimer();
    ...
    public void quitFullScreen() {
        ...
        LCBMediaManager.intance().mediaPlayer.pause();
        LCBMediaManager.intance().mediaPlayer.setDisplay(null);
        LCBMediaManager.intance().revertUuid();
    ...
    public static void releaseAllVideos() {
        if (!isClickFullscreen) {
            LCBMediaManager.intance().mediaPlayer.stop();
            LCBMediaManager.intance().setUuid("");
            LCBMediaManager.intance().setUuid("");
    ...
    private void loadMp3Thum() {
        Utils.loadImage(context,thumb,ivCover);
    }
